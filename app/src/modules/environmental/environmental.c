...
static void environmental_wdt_callback(int channel_id, void *user_data)
{
...
static void environmental_module_thread(void)
{
	int err;
	int task_wdt_id;
	...
	task_wdt_id = task_wdt_add(wdt_timeout_ms, environmental_wdt_callback, (void *)k_current_get());
	if (task_wdt_id < 0) {
...
K_THREAD_DEFINE(environmental_module_thread_id,
		...
		environmental_module_thread, NULL, NULL, NULL, K_LOWEST_APPLICATION_THREAD_PRIO, 0, 0);

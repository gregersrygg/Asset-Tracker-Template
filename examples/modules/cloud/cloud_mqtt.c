				  ...
				  const struct cloud_state *state_object)
{
	...
	struct mqtt_publish_param param = {
		.message.payload.data = msg->payload.buffer,
	...
	LOG_DBG("MQTT Publish Details:\n"
		...
		"QoS: %d\n"
		"Message ID: %d",
		param.message.payload.len, param.message.payload.data,
		param.message.payload.len,
		param.message.topic.topic.size, param.message.topic.topic.utf8,
